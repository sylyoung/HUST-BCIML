...
DeepTransferEEG whose value is **algorithm coverage**: ~30 of the lab's EEG-decoding methods,
plus the standard external baselines, ported onto **one stage architecture** and measured
head-to-head under a **single controlled protocol**. The full 3-seed benchmark runs on three
MOABB motor-imagery EEG datasets — **BNCI2014001** (9 subjects, 22 ch), **BNCI2014002**
(14, 15 ch) and **BNCI2015001** (12, 13 ch) — all cross-subject, leave-one-subject-out. A
...
configuration (**EA · no-aug · EEGNet · Linear · ERM**), so every row differs from its baseline
in one way and the comparison is fair. A separate **ensemble** axis aggregates several models.
...
| **Augmenter** | `ChannelReflection`, `CSDA`, `Identity` |
| **Backbone**  | `EEGNet`, `ShallowConvNet`, `DeepConvNet`, `EEGConformer`, `DBConformer`, `IFNet`, `CSPNet`, `TIEEEGNet`, `KDFNet` |
| **Head**      | `Linear` |
...
**Ensembles** — black-box multi-seed combiners over K seeds of a base algorithm (hard majority
...
Faithful **negative** results are kept and explained, not hidden.
...
- **`MOABBAdapter`** — downloads BNCI via MOABB into that same layout, then loads it. The path
...
target once real numbers exist, and a card. Each new file carries a standard header — author,
