#!/bin/bash
# Delta run: ISFDA (imbalanced source-free DA, online-TTA variant) on BNCI2014001
...
# Usage: sweep_isfda.sh GPU_ID ITR   (ITR = number of seeds, seeds start at 1)
set -u
GPU=${1:-5}
ITR=${2:-1}
...
echo "===== ISFDA (imbalanced source-free DA, online TTA) ====="
python -m hustbciml.run --algorithm ISFDA $COMMON
echo "SWEEP_ISFDA_DONE"
