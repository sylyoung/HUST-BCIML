...
class EEGConformer(Backbone):
    ...
    def __init__(self, n_chans: int, n_times: int, n_classes: int, sfreq: float,
        ...
        super().__init__()
        # convolutional tokenizer (temporal + spatial conv, band-power-style pool)
        self.patch = nn.Sequential(
            nn.Conv2d(1, emb, (1, 25)),
            nn.Conv2d(emb, emb, (n_chans, 1)),
            nn.BatchNorm2d(emb),
            nn.ELU(),
    ...
    def _embed(self, x: torch.Tensor) -> torch.Tensor:
        z = self.patch(x)                 # (B, emb, 1, tokens)
        z = self.proj(z)
        z = z.squeeze(2).transpose(1, 2)  # (B, tokens, emb)
        z = self.transformer(z)
        return z.flatten(1)               # (B, tokens * emb)
