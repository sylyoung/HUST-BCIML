...
- **2026-08-06（v1.6.5）** 署名修正：BFT 的「实现者」改为实际的两名实现人 Jiayi Ouyang 与 Siyang Li（Ouyang 在前），不再列论文全部作者。**所有测量数值不变**。

- **2026-08-06（v1.6.4）** 文档修复：两个 README 的「更新日志」折叠块恢复默认折叠。v1.6.0 重写时误删了 `<details>` 开标签，导致整段默认展开。**所有测量数值不变**。
