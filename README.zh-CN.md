...
中文改用实验室 公众号 稿件的行文方式。同时订正了核心项目卡片上已过时的方法计数，
...
这是一个自包含的框架，围绕单一命令行入口和一个自动扫描的插件注册表构建。在这条可组合的流水线上，它重新实现了 **58 种脑电解码方法**，涵盖数据对齐、数据增强、网络骨干与迁移学习，另有 **14 种集成聚合方法**单独计数（它们聚合多个已训练模型，而不是组成一条流水线），并在**单一受控评估协议**下对它们正面比较，同时为每一个报告数值配上逐方法的复现记录。
...
- **仅源域训练，配合目标域无标签对齐**：**ERM**（无迁移基线）、**MDMAML (lab)**、**ABAT (lab)**、**PAT (lab)**。训练阶段不使用目标域数据，也不使用任何目标标签。但这四种方法都组合了 `aligner: EA`，而欧氏对齐会用留出被试自己的无标签试次估计其白化参考矩阵，再做预测。这是标准的 EA 流程，不构成标签泄漏，但它是直推式的，而不是零样本，因此原先完全不用目标域的说法并不准确。
...
#### 超参数选择：它保证了什么，又没有保证什么

我们用一个小网格搜索了学习率、训练时长和各方法自身的损失权衡（loss trade-offs），获胜配置的三种子测试数值**只有在优于此前数值时**才替换。打分用了两种信号，它们给出的保证并不一样。

* **源域验证选择**（`select="val"`，用于源模型的各项超参数：ABAT、CSDA，以及网络骨干表中各架构的学习率）。分数来自留出的一部分**源**被试的准确率，任何目标域数据都不参与。这一种是干净的。

* **开发被试选择**（`select="dev"`，用于那些在源域验证信号上分不出高下的自适应阶段超参数：ASFA、Tent、BFT、DJP-MMD、MDMAML、MSDT、LSFT、MVCNet）。做法是从整个被试群体中挑出分布较开的三名被试当作伪目标，各自按留一被试准确率打分，**用的是他们的真实标签**，再据此选出一个全局配置。这三名被试随后仍然计入所报告的平均值。也就是说，对这八种方法而言，所报告的九（或十四、十二）折里有三折同时充当了选择信号。训练阶段没有用到任何目标域标签，选出的也是一个全局值而不是逐折的值。但这仍然是常见的那种在被试子集上选超参数的做法，而不是纯源域信号，所以这里把它写明，而不是让读者以为是前一种。
