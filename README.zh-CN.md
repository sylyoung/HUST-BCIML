...
- **2026-09-12（v1.6.17）** 总览页方法名后面的小三角标记已经去掉，它在那个字号下看起来像一个多余的点。
- **2026-09-12（v1.6.16）** 总览页的方法名不再直接跳转到代码，而是给出三个去向，分别是它的代码、论文和排行榜中的测量结果。
...
- **2026-08-28（v1.6.9）** 论文索引更新至 273 篇，三篇预印本已正式发表，每条记录都补齐了 DOI。
...
一个静态网页应用，并列呈现基准排行榜和覆盖实验室 **309 篇论文**的可检索**论文到代码总览**，其中 97 篇有公开代码。它可以作为本地文件直接打开，也可以由 GitHub Pages 托管，**无需构建步骤**。
...
- **唯一权威数据源**，位于 [`gallery/data/`](gallery/data/)，包括 `publications.yml`（309 篇论文）、`lab.yml`（简介、核心项目、精选仓库）与 `benchmark.yml`（受控比较排行榜）。
