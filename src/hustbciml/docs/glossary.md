...
| **transductive** | A method that reads the *unlabeled* target before or during producing its prediction. Two distinct forms, and two distinct flags. `Strategy.uses_target = True` marks a strategy that reads the target during source *training* (DANN, CDAN, MCC, DAN, JAN, MDD, DJP-MMD); the protocol supplies it as `ctx.target_unlabeled` with labels masked. `Strategy.transductive_predict = True` marks one that learns projections or pseudo-labels from the whole unlabeled target batch inside `predict` (MEKT, LSFT, MSDT) — those never touch `ctx.target_unlabeled`, so reading only `uses_target` would classify them as non-transductive. |
| **transductive alignment** | Worth stating separately, because it applies to *every* EA row, including the ones grouped as source-only. Under leave-one-subject-out, `EA.transform` on a domain it never saw during `fit` estimates that subject's whitening reference from the subject's own trials. No labels are used and it is the standard EA protocol, but it means the held-out subject's unlabelled data is seen before prediction. The results are therefore transductive rather than zero-shot. |
