...
MCC leads on every dataset (79.04 / 80.88 / 78.53), and the strong adaptation cluster of
CDAN, T-TIME, DELTA and ISFDA sits +3 to +6 over ERM throughout. The online and source-free
...
## Augmentation (no-alignment regime)
_EEGNet + ERM. Vary the augmenter. Baseline = no augmentation (no-alignment EEGNet). Held at
no-alignment (aligner: Identity), because these are electrode-space transforms that rearrange
channels and must precede any spatial whitening._
...
the two new datasets (72.71 vs 74.40, and 72.00 vs 73.19). Riemann-MDM (minimum distance to the
Riemannian mean) trails on all three, by a widening margin on the higher-channel-count
datasets. A well-tuned CSP-LDA pipeline stays competitive with a deep
...
ensemble. (An across-subject std is omitted from the table because it is large. The 2a
...
behind, and no weighting scheme beats voting on these near-identical strong models. On
...
NeurIPS 2009), M-MSR (Ma & Olshevsky, NeurIPS 2020), Wawa (crowd-kit heuristic), and MACE (Hovy et al.,
...
protocol: each single-source EA-EEGNet trains on one subject only (BNCI2014002 uses 100
training-run trials, BNCI2015001 uses 200 first-session trials), Centralized Training pools
...
reference range, and per-method note, in `src/hustbciml/tests/repro/repro_targets.yaml`.
