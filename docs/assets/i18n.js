  ...
  window.I18N = {
    zh: {
      ...
      "The aggregation stage, in a fully decentralized and privacy-preserving setting. Each source subject trains three different learners, i.e., tangent-space logistic regression, CSP-Net and EEGConformer, on its own data alone, and shares only its hard predicted labels on the target, never the model weights or the raw EEG. A combiner then fuses the resulting (N−1)×3 label votes into a single prediction, without any target label. One learner is taken from each of three model families, i.e., a Riemannian linear model, a convolutional network and a self-attention network, so that the votes a single subject contributes are as mutually decorrelated as this menu allows. Because only the hard votes are observed, the task reduces to estimating the reliability of each learner in the absence of the ground truth. Two non-ensemble references bound the task, and the combiners are grouped below them.":
        "聚合模块，运行于完全去中心化的隐私保护场景。每名源被试仅使用自身数据训练三个不同的学习器，即切空间逻辑回归、CSP-Net 和 EEGConformer，并且只共享其在目标域上的硬预测标签，不共享模型权重和原始脑电。组合器随后将由此得到的 (N−1)×3 个标签投票融合为单一预测，全过程没有任何目标域标签。三个学习器分别取自三类模型族，即黎曼线性模型、卷积网络和自注意力网络，使同一名被试贡献的投票在现有方法范围内尽可能互不相关。由于仅能观测到硬投票，该任务归结为在缺乏真值的条件下估计各学习器的可靠程度。两个非集成参照界定了该任务的上下界，各组合器分组列于其下。",
